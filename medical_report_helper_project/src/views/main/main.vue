...
const leftTableData = ref([
  {
    ...
    cellMorphology: {},
    pic: ['https://picsum.photos/300/200?random=1', 'https://picsum.photos/300/200?random=2'],
    otherInfo: {},
  },
  {
    name: '13',
...
const handleLeftTableRowClick = async (row) => {
  ...
  // 保存当前选中行
  currentSelectedRow.value = row // 添加这行代码，确保当前选中行正确更新

  // 清空表格数据
  if (row && row.cellData) {
    tableData.value = Array.isArray(row.cellData) ? row.cellData : [row.cellData]
  ...
  if (row) {
    ...
    form.dept = row.dept || ''
    form.orderDoc = row.orderDoc || ''
    form.boneSource = row.boneSource || ''
    form.sendDate = row.sendDate ? new Date(row.sendDate) : ''
    form.reportDoc = row.reportDoc || ''
    form.auditDoc = row.auditDoc || ''
    form.reportDate = row.reportDate ? new Date(row.reportDate) : ''
...
const handleSave = async () => {
  ...
  for (const field of requiredFields) {
    ...
    const value =
      field.from === 'selectedRow' ? currentSelectedRow.value[field.key] : form.value[field.key]
...
const getLabel = (options, value) => {
  ...
  return item ? item.label : value // 找不到时显示原始value
}

const handleExport = () => {
  const formattedForm = {
    ...form,
    dept: getLabel(deptOptions, form.dept),
    hospital: getLabel(hospitalOptions, form.hospital),
    orderDoc: getLabel(docOptions, form.orderDoc),
    reportDoc: getLabel(docOptions, form.reportDoc),
    auditDoc: getLabel(docOptions, form.auditDoc),
    boneSource: getLabel(sourceOptions, form.boneSource),

    sendDate: form.sendDate ? dayjs(form.sendDate).format('YYYY-MM-DD') : '',
    reportDate: form.reportDate ? dayjs(form.reportDate).format('YYYY-MM-DD') : ''
  }
  ...
  const exportData = {
    ...
    // 图片列表
    imageList: JSON.stringify(imageList.value || []),
    // 医生意见
    advice: textarea.value.trim() || '无',
  }
  router
    .push({
      name: 'exportPreview',
  ...
  <div class="common-layout">
    <el-container style="flex-direction: column">
      ...
      <el-container style="flex-direction: row">
        ...
        <RightBarComponent :table-data="tableData" />
      </el-container>
    </el-container>
  </div>
</template>
